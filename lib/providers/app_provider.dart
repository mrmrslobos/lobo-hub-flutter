...
class AppProvider extends ChangeNotifier {
  ...
  /// Await after chore / completion changes so Supabase stays in sync.
  Future<void> syncChoresNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilyChoresToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncChoresNow: $e');
    }
  }

  /// Await after poll / vote changes so Supabase stays in sync.
  Future<void> syncPollsNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilyPollsToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncPollsNow: $e');
    }
  }

  /// Await after occasion (special_dates) add/edit/delete so Supabase stays in sync.
  Future<void> syncOccasionsNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilySpecialDatesToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncOccasionsNow: $e');
    }
  }
  ...
  /// Await after workout / weight / plan / PR changes so Supabase stays in sync.
  Future<void> syncFitnessNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilyFitnessToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncFitnessNow: $e');
    }
  }

  /// Await after period / symptom log changes so Supabase stays in sync.
  Future<void> syncPeriodTrackerNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilyPeriodDataToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncPeriodTrackerNow: $e');
    }
  }
  ...
  /// Await after rewards / redemptions (or related catalog) changes.
  Future<void> syncRewardsNow() async {
    final fam = _activeFamily;
    if (fam == null || !SupabaseService.isConfigured) return;
    try {
      await DatabaseService.pushFamilyRewardsToCloudNow(_db, fam.id);
      _broadcastChange();
    } catch (e) {
      debugPrint('[AppProvider] syncRewardsNow: $e');
    }
  }
